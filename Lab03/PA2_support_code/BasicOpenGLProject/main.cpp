...
std::vector <glm::vec4> lines_color;
int n = 12;
float R = 0.5;
float r = 0.2;
bool flat = false;
...
void axis_vectors()
{
		...
		float Axisx1, Axisy1, Axisz1 = 0;
		std::vector<std::vector<std::vector<float>>> vector_3d;
		std::vector<std::vector<std::vector<float>>> vector_Rr3;
		for (int i = 0; i <= n; i++)
		{
			std::vector<std::vector<float>> v2d;
			std::vector<std::vector<float>> vRr2;
			for (int j = 0; j <= n; j++)
			{
				std::vector<float> v1d;
				std::vector<float> vRr1;
				for (int k = 0; k < b; k++)
				{
					if (k == 0)
					{
						Axisx = (R + r * cos(theta)) * cos(phi);
						Axisx1 = (cos(theta)) * cos(phi);
						v1d.push_back(Axisx);
						vRr1.push_back(Axisx1);
					}
					if (k == 1)
					{
						Axisy = (R + r * cos(theta)) * sin(phi);
						Axisy1 = (cos(theta)) * sin(phi);
						v1d.push_back(Axisy);
						vRr1.push_back(Axisy1);
					}
					if (k == 2)
					{
						Axisz = r * sin(theta);
		...
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			{
				for (int k = 0; k < 6; k++)
				{
					...
					if (k == 5) {
						x = (vector_3d[i + 1][j + 1][0]);
						y = (vector_3d[i + 1][j + 1][1]);
						z = (vector_3d[i + 1][j + 1][2]);
						w = (vector_3d[i + 1][j + 1][3]);
						white.push_back(glm::vec4(1.0, 1.0, 1.0, 1.0));
						points.push_back(glm::vec4(x, y, z, w));
						x = (vector_Rr3[i + 1][j + 1][0]);
						y = (vector_Rr3[i + 1][j + 1][1]);
						z = (vector_Rr3[i + 1][j + 1][2]);
						w = (vector_Rr3[i + 1][j + 1][3]);
						Origin.push_back(glm::vec4(x, y, z, w));
					}
				}
			}
		...
		if (flat == true) {
			for (int i = 0; i < points.size(); i += 3)
			{
				...
				if (toggle == true) {
					lines.push_back(glm::vec4((A.x + B.x + C.x) / 3, (A.y + B.y + C.y) / 3, (A.z + B.z + C.z) / 3, 1.0));
					lines.push_back(glm::normalize(glm::vec4((ABxAC), 1.0)) + glm::vec4((A.x + B.x + C.x) / 3, (A.y + B.y + C.y) / 3, (A.z + B.z + C.z) / 3, 1.0));
				}
				lines_color.push_back(glm::vec4(1.0, 1.0, 0.0, 0.0));
				lines_color.push_back(glm::vec4(1.0, 1.0, 0.0, 0.0));
		...
		if (smooth == true) {
			for (int i = 0; i < points.size(); i++)
			{
				glm::vec3 V = points[i];
				glm::vec3 C = Origin[i];
				vert_Normal.push_back(glm::vec4(C, 1.0));
				if (toggle == true) {
					lines.push_back(glm::vec4(V, 1.0));
					lines.push_back((glm::vec4(C, 1.0) * 0.25f + glm::vec4(V, 1.0)));
					lines_color.push_back(glm::vec4(1.0, 1.0, 0.0, 0.0));
					lines_color.push_back(glm::vec4(1.0, 1.0, 0.0, 0.0));
				}
			}
...
void CreateShaders( void )
{
    ...
    // Renders using Flat projection
	FlatShader.Create("./shaders/persplight.vert", "./shaders/persplight.frag");
}
...
void CreateAxisBuffers( void )
{
	...
	glBufferData(GL_ARRAY_BUFFER, lines_color.size() * sizeof(glm::vec4), &lines_color[0], GL_STATIC_DRAW);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(1);
...
void display_func( void )
{
	...
	if (flat == true || smooth == true) {
		FlatShader.Use();
		FlatShader.SetUniform("projectionMatrix", glm::value_ptr(PerspProjectionMatrix), 4, GL_FALSE, 1);
		FlatShader.SetUniform("viewMatrix", glm::value_ptr(PerspViewMatrix), 4, GL_FALSE, 1);
		FlatShader.SetUniform("modelMatrix", glm::value_ptr(PerspModelMatrix), 4, GL_FALSE, 1);
	}
